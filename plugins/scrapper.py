...
async def listmv(c, m):
    ...
    if querys == "":
        ...
        link = f"https://www.1tamilmv.tf/index.php?/search/&q={querys}&search_and_or=and&search_in=titles&sortby=relevancy"
...
async def lists(c, m):
    ...
    if querys == "":
        ...
        link = f"https://1tamilblasters.mov/index.php?/search/&q={querys}&search_and_or=and&search_in=titles&sortby=relevancy"
        txt = await m.reply_text(f"Searching for: {querys} 🔍")
        driver.get(link)
        await asyncio.sleep(5)
        title = driver.title
...
async def ss(bot, message):
    ...
    name = ''.join(random.choices(string.ascii_uppercase +
                                  string.digits, k=N))
    driver.get("https://www.1tamilmv.tf/")
    photo = name + ".png"
    ...
    name = ''.join(random.choices(string.ascii_uppercase +
                                  string.digits, k=N))
    driver.get("https://1tamilblasters.mov/")
